import ru.geekbrains.java1.lesson2.Animal;
...
public class Dog extends Animal {
    ...
    public boolean swim(int distance) {
        ...
        public boolean jump(int height) {
            if (this.maxSwimDistance >= height) {
                System.out.printf("Собака перепрыгнула препятствие %d. Max = %d%n", height, maxJumpHeight);
                return true;
            }

            System.out.printf("Собака не смогла переплыть дистанцию %d. Max = %d%n", height, maxJumpHeight);
            return false;
